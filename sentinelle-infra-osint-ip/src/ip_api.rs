...
impl<M: MetricsPort> IpApiClient<M> {
    ...
    pub async fn fetch(&self, ip: IpAddr) -> Result<Option<IpIntelligence>, IpIntelError> {
        ...
        let resp = self
            ...
            .await
            .map_err(|e| {
                eprintln!("IP-API request failed: {}", e);
        ...
        if !ok {
            eprintln!("IP-API returned status: {}", resp.status());
        ...
        let body: ApiResponse = resp
            ...
            .await
            .map_err(|e| {
                eprintln!("IP-API JSON parsing failed: {}", e);
                IpIntelError::ProviderFailure
            })?;
        ...
        if body.status == "success" {
            ...
            eprintln!("IP-API returned status: {}, message: {:?}", body.status, body.message);
            Ok(None)
        }
    }
}

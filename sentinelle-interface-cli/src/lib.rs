...
use sentinelle_infra_osint_ip::EnrichedIpIntelligenceEngine;
use sentinelle_infra_osint_mail::MailOsintEngine;
use sentinelle_infra_osint_social::SocialOsintEngine;
use sentinelle_infra_osint_advanced::{AdvancedOsintEngine, TargetType};
...
impl App {
    fn new() -> Result<Self, io::Error> {
        let cache = SqliteCache::new("sentinelle_cache.db")
            .or_else(|_| SqliteCache::new(":memory:"))
            .map_err(|_| io::Error::new(io::ErrorKind::Other, "Failed to initialize cache"))?;
    ...
    fn log_line(&mut self, line: impl Into<String>) {
        ...
        // Limiter la longueur des lignes pour éviter la déformation de l'interface
        if line.len() > 80 {
            line.truncate(77);
            line.push_str("...");
        }
...
fn ui(f: &mut ratatui::Frame, app: &App) {
    ...
    // Header ASCII
    let banner_lines: Vec<ratatui::text::Line> = BANNER
        .iter()
        .map(|line| ratatui::text::Line::from(Span::styled(
            *line,
        ...
        let list_items: Vec<ListItem> = items
            ...
            .map(|(i, item)| {
                if i == app.selected_menu {
                    ListItem::new(ratatui::text::Line::from(Span::styled(
                        *item,
                        Style::default().add_modifier(Modifier::REVERSED),
                    )))
                } else {
                    ListItem::new(ratatui::text::Line::from(Span::raw(*item)))
                }
            })
            .collect();
    ...
    let prompt = match app.view {
        ...
        View::AdvancedOsintInput => "Cible OSINT Avancé (email/domain/ip/username): ",
    ...
    match &app.report {
        ...
        ReportKind::None => {
            ...
            let log_lines: Vec<ratatui::text::Line> = app
                ...
                .skip(start)
                .take(end.saturating_sub(start))
                .map(|l| ratatui::text::Line::from(Span::raw(l.as_str())))
                .collect();
    ...
    rows.extend(
        filtered_activity
            ...
            .map(|ev| {
                let ev = *ev;
                let dt = chrono::DateTime::from_timestamp(ev.ts as i64, 0)
    ...
    let activity_widget = Table::new(rows, widths)
        .header(header)
        ...
        View::MainMenu => match key.code {
            ...
            KeyCode::Enter => {
                ...
                match app.selected_menu {
                    ...
                    8 => app.view = View::AdvancedOsintInput,
                    9 => return Ok(false),
                    _ => {}
                }
            }
        ...
        _ => match key.code {
            ...
            KeyCode::Enter => {
                ...
                let res = match app.view {
                    ...
                    View::AdvancedOsintInput => { run_advanced_osint(app, &input); ("advanced_osint", input.clone(), "advanced") }
...
fn run_ip(app: &mut App, input: &str) {
    ...
    if input.is_empty() {
        app.log_line("Erreur: Adresse IP vide");
        return;
    }
    
    if input.len() > 45 { // IPv6 max length
        app.log_line("Erreur: Adresse IP trop longue");
        return;
    }
    ...
    let ip: IpAddr = match input.parse() {
        Ok(ip) => ip,
        Err(_) => {
            app.log_line(format!("Erreur: Adresse IP invalide: {}", input));
            return;
        }
    };
    ...
    if ip.is_unspecified() || ip.is_loopback() || ip.is_multicast() {
        app.log_line(format!("Erreur: Adresse IP non autorisée: {} (loopback/multicast/unspecified)", ip));
        return;
    }

    // Validation des plages privées pour OSINT
    match ip {
        IpAddr::V4(v4) => {
            if v4.is_private() || v4.is_link_local() || v4.is_broadcast() {
                app.log_line(format!("Avertissement: Adresse IP privée/locale: {}", ip));
            }
        }
        IpAddr::V6(v6) => {
            if v6.is_loopback() || v6.is_unspecified() {
                app.log_line(format!("Erreur: Adresse IPv6 non autorisée: {}", ip));
                return;
            }
        }
    }
    ...
    let ip_engine = EnrichedIpIntelligenceEngine::new_with_default_client(metrics);
    let usecase = RunIpIntelligence::new(&ip_engine);

    app.log_line(format!("🔍 Analyzing IP: {}", ip));
    match usecase.execute(ip) {
        Ok(intel) => {
            app.log_line(format!("✅ Enhanced IP Intelligence for {}", intel.ip));
            if let Some(country) = intel.country {
                app.log_line(format!("  🏳️  Country     : {}", country));
            }
            if let Some(city) = intel.city {
                app.log_line(format!("  🏙️  City        : {}", city));
            }
            if let Some(isp) = intel.isp {
                app.log_line(format!("  🏢 ISP         : {}", isp));
            }
            if let Some(asn) = intel.asn {
                app.log_line(format!("  🔢 ASN         : {}", asn));
            }
            if let Some(hostname) = intel.ptr {
                app.log_line(format!("  🌐 Hostname    : {}", hostname));
            }
            if let Some(lat) = intel.latitude {
                if let Some(lon) = intel.longitude {
                    app.log_line(format!("  📍 Location    : {:.4}, {:.4}", lat, lon));
                }
            }
            if let Some(tz) = intel.timezone {
                app.log_line(format!("  🕐 Timezone    : {}", tz));
            }
        }
        Err(e) => app.log_line(format!("❌ IP Intelligence error: {}", e)),
    }
}
fn run_mail(app: &mut App, input: &str) {
    ...
    let input = input.trim();
    if input.is_empty() {
        app.log_line("Erreur: Email vide");
        return;
    }
    
    if input.len() > 254 { // RFC 5321 limit
        app.log_line("Erreur: Email trop long (max 254 caractères)");
        return;
    }
    
    // Validation basique de format avant parsing
    if !input.contains('@') || input.starts_with('@') || input.ends_with('@') {
        app.log_line(format!("Erreur: Format email invalide: {}", input));
        return;
    }
    
    // Vérification de caractères dangereux
    if input.contains('\0') || input.contains('\n') || input.contains('\r') {
        app.log_line("Erreur: Email contient des caractères interdits");
        return;
    }
    ...
    let email = match Email::parse(input) {
        Ok(e) => e,
        Err(e) => {
            app.log_line(format!("Erreur: Email invalide: {}", e));
            return;
        }
    };
...
fn run_social(app: &mut App, input: &str) {
    ...
    if username.is_empty() {
        app.log_line("Erreur: Username vide");
        return;
    }
    
    if username.len() > 50 { // Limite raisonnable
        app.log_line("Erreur: Username trop long (max 50 caractères)");
        return;
    }
    ...
    if username.contains('\0') || username.contains('\n') || username.contains('\r') 
        ...
        app.log_line("Erreur: Username contient des caractères interdits");
        return;
    }
    
    // Validation basique de format username
    if !username.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-' || c == '.') {
        app.log_line("Erreur: Username contient des caractères non autorisés (seuls a-z, A-Z, 0-9, _, -, . sont autorisés)");
        return;
    }
...
fn run_sigint_tcp(app: &mut App, input: &str) {
    ...
    let input = input.trim();
    if input.is_empty() {
        app.log_line("Erreur: Input vide");
        return;
    }
    
    let parts: Vec<&str> = input.split(':').collect();
    if parts.len() != 2 {
        app.log_line("Erreur: Format attendu: IP:PORT, ex: 1.2.3.4:443");
        return;
    }
    ...
    if ip_str.is_empty() || ip_str.len() > 45 {
        app.log_line("Erreur: Adresse IP invalide");
        return;
    }
    ...
    let ip: IpAddr = match ip_str.parse() {
        Ok(ip) => ip,
        Err(_) => {
            app.log_line(format!("Erreur: Adresse IP invalide: {}", ip_str));
            return;
        }
    };

    // Validation de sécurité IP
    if ip.is_unspecified() || ip.is_loopback() || ip.is_multicast() {
        app.log_line(format!("Erreur: Adresse IP non autorisée: {}", ip));
        return;
    }
    ...
    let port: u16 = match port_str.parse() {
        Ok(p) => p,
        Err(_) => {
            app.log_line(format!("Erreur: Port invalide: {}", port_str));
            return;
        }
    };

    // Validation de sécurité PORT
    if port == 0 {
        app.log_line("Erreur: Port 0 non autorisé");
        return;
    }
    
    // Avertissement pour ports privilégiés
    if port < 1024 {
        app.log_line(format!("Avertissement: Port privilégié: {}", port));
    }
...
fn run_advanced_osint(app: &mut App, input: &str) {
    // Validation stricte de l'input
    let input = input.trim();
    if input.is_empty() {
        app.log_line("Erreur: Cible vide");
        return;
    }
    
    if input.len() > 254 {
        app.log_line("Erreur: Cible trop longue (max 254 caractères)");
        return;
    }
    
    // Vérification de caractères dangereux
    if input.contains('\0') || input.contains('\n') || input.contains('\r') {
        app.log_line("Erreur: Cible contient des caractères interdits");
        return;
    }

    // Déterminer le type de cible automatiquement
    let target_type = if input.contains('@') {
        TargetType::Email
    } else if input.parse::<std::net::IpAddr>().is_ok() {
        TargetType::IpAddress
    } else if input.contains('.') && !input.contains(' ') {
        TargetType::Domain
    } else if input.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-' || c == '.') {
        TargetType::Username
    } else if input.chars().all(|c| c.is_ascii_digit() || c == '+' || c == '-' || c == ' ' || c == '(' || c == ')') {
        TargetType::PhoneNumber
    } else {
        TargetType::Organization
    };

    let cache_key = format!("advanced_osint:{}:{:?}", input, target_type);
    if app.cache_load(&cache_key) {
        app.log_line(format!("(cache) Investigation OSINT avancée déjà effectuée pour {}", input));
        return;
    }

    app.log.clear();
    app.log_line(format!("🚀 Investigation OSINT Avancée: {}", input));
    app.log_line(format!("🎯 Type détecté: {:?}", target_type));

    // Créer le moteur OSINT avancé
    let engine = AdvancedOsintEngine::new();
    
    // Utiliser tokio pour l'exécution asynchrone
    let rt = match tokio::runtime::Runtime::new() {
        Ok(rt) => rt,
        Err(e) => {
            app.log_line(format!("❌ Erreur runtime async: {}", e));
            return;
        }
    };
    ...
    match rt.block_on(engine.investigate(input, target_type)) {
        Ok(result) => {
            ...
            app.log_line(format!("📊 Confiance: {:.1}% | Risque: {}", 
                result.confidence_score * 100.0, result.risk_assessment.overall_risk));
            
            // Résumé compact des résultats
            app.log_line(format!("🔍 Google Dorks: {} requêtes exécutées", result.google_dorks.len()));
            for dork in result.google_dorks.iter().take(2) {
                app.log_line(format!("  • {} ({} résultats)", dork.query, dork.total_results));
            }
            
            app.log_line(format!("🕷️  Web Scraping: {} sites analysés", result.web_scraping.len()));
            for scrape in result.web_scraping.iter().take(1) {
                app.log_line(format!("  • {} ({} emails, {} liens)", 
                    scrape.url, scrape.emails.len(), scrape.links.len()));
            }
            
            app.log_line(format!("📱 Social Intel: {} plateformes", result.social_intelligence.len()));
            for social in result.social_intelligence.iter().take(2) {
                let verified = if social.verified { "✓" } else { "✗" };
                app.log_line(format!("  • {} {} ({})", social.platform, verified,
                    social.followers.map(|f| f.to_string()).unwrap_or_else(|| "N/A".to_string())));
            }
            
            if !result.threat_intelligence.is_empty() {
                app.log_line(format!("⚠️  Menaces: {} détectées", result.threat_intelligence.len()));
                for threat in result.threat_intelligence.iter().take(1) {
                    app.log_line(format!("  • {} ({})", threat.threat_type, threat.severity));
                }
            } else {
                app.log_line("✅ Aucune menace détectée");
            }
            
            // Métriques de confiance
            app.log_line("🧠 Métriques de confiance:");
            for (source, confidence) in result.data_fusion.confidence_metrics.iter().take(4) {
                app.log_line(format!("  • {}: {:.0}%", source, confidence * 100.0));
            }
            
            // Relations détectées
            if !result.data_fusion.relationships.is_empty() {
                app.log_line(format!("🔗 Relations: {} détectées", result.data_fusion.relationships.len()));
                for rel in result.data_fusion.relationships.iter().take(2) {
                    app.log_line(format!("  • {} -> {} ({:.0}%)", 
                        rel.entity1, rel.entity2, rel.strength * 100.0));
                }
            }
            
            // Recommandations
            if !result.risk_assessment.mitigation_recommendations.is_empty() {
                app.log_line("💡 Recommandations:");
                for rec in result.risk_assessment.mitigation_recommendations.iter().take(2) {
                    app.log_line(format!("  • {}", rec));
                }
            }
            
            app.log_line("🎯 Investigation OSINT Avancée terminée");
        }
        Err(e) => {
